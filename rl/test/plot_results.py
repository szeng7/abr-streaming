...
ax.legend(SCHEMES_REW, loc='upper right')
...
for l in time_all[SCHEMES[0]]:
	...
	if schemes_check:
		...
		for scheme in SCHEMES:
	        ...
	        ax.legend(SCHEMES_REW, loc=4, bbox_to_anchor=(0.8, 3.1), ncol=int(np.ceil(len(SCHEMES) / 2.0)))
	        #ax.legend(SCHEMES_REW, loc=4, bbox_to_anchor=(0.5, -0.1), ncol=int(np.ceil(len(SCHEMES) / 2.0)))
		#plt.show()
		plt.savefig("sim_rl_results.png")
